...
Expanded from $7$ to $9$ dilated blocks with $96$ channels ($\sim$1.2M params, up from 409K). Receptive field: $1{,}197$ steps---more than $2\times$ the input window.
...
\begin{table}[h]
\caption{Training Progress---v2 Models (In Progress)}
\label{tab:training_status}
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Epochs Completed} & \textbf{Est.\ Time/Epoch} & \textbf{Parameters} \\
\midrule
LSTM-CNN v2 & $29 / 120$ & $\sim$6--8~min & $\sim$1{,}200{,}000$ \\
TCN v4 (U-Net) & $59 / 120$ & $\sim$4--6~min & $\sim$1{,}200{,}000$ \\
\midrule
\multicolumn{4}{l}{\textit{Previous baselines (completed):}} \\
WaveNet (old) & $29 / 29$ & $\sim$3--4~min & $409{,}100$ \\
TCN v3 (old) & $59 / 59$ & $\sim$4--6~min & $1{,}807{,}563$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent Training is conducted on CPU (Apple M-series). Both v2 models use Adam optimizer (lr=$3\times 10^{-4}$), batch size $64$, gradient clipping at $1.0$, and early stopping with patience $20$.
...
3 & Architecture: LSTM-CNN v2 \& TCN v4 & 4~min \\
4 & Quantitative Results \& Training Status & 3~min \\
5 & Explainability: Gradient$\times$Input, Occlusion (Q2) & 3~min \\
6 & UQ: MC Dropout, CI Meaning (Q2, Q5) & 4~min \\
7 & Literature Comparison (Q3) & 2~min \\
8 & Domain Knowledge Connection (Q4) & 2~min \\
9 & Future Methods \& Conclusions (Q6) & 2~min \\
\midrule
